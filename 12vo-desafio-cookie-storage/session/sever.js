...
app.get('/', (req, res) => {
    if (req.query.name) {  // localhost:8080/?name=Juan  SE INGRESO UN NOMBRE EN EL QUERY???
        if (req.session.visitas) {  //HAY UN REGISTRO DE VISITAS???
            req.session.visitas++; //LE SUMMO UNO A NUESTRO CONTADOR 
    ...
    } else {  //SI NO LE SGREGAS NOMBRE LA RUTA 
...
app.get('/olvidar', (req, res) => {
    if (req.query.name) {  //SE INGRESO UN NOMBRE EN EL QUERY??? 
        req.session.destroy();
        res.send(`Hasta luego ${req.query.name}`);

    } else { ///SINO MOSTRAR ESTE MENSAJE
